...
void print_gss_parameters(struct sift_scalespace* ss, const char* basename)
{
    char filename[FILENAME_MAX];
    sprintf(filename, "gss_params_%s.txt", basename);
    ...
    fprintf(file, "# Gaussian Scale Space Parameters\n");
    fprintf(file, "# Format: octave shape_h shape_w delta sigma1 sigma2 ... sigmaN\n");
    fprintf(file, "# Generated by sift_cli with -verb_params flag\n\n");
    
    for(int o = 0; o < ss->nOct; o++) {
        struct octa* octave = ss->octaves[o];
        fprintf(file, "%d %d %d %.6f", o, octave->h, octave->w, octave->delta);
        for(int s = 0; s < octave->nSca; s++) {
            fprintf(file, " %.6f", octave->sigmas[s]);
        }
        fprintf(file, "\n");
    }
    
    fclose(file);
    printf("GSS parameters written to: %s\n", filename);
}
...
void print_usage()
{
    ...
    fprintf(stderr, "   -verb_params label   flag to output the scale space parameters (shapes, deltas, sigmas)  \n");
}
                         ...
                         char* label_params)
{
    ...
    isfound = pick_option(&argc, &argv, "verb_params", val);
    if (isfound ==  1){
        *flag_params = 1;
        strcpy(label_params, val);
    }
    if (isfound == -1)    return EXIT_FAILURE;
...
int main(int argc, char **argv)
{
    ...
    int flagverb_keys = 0;
    int flagverb_ss = 0;
    int flagverb_params = 0;
    char label_ss[256];
    char label_keys[256];
    char label_params[256];
    strcpy(label_ss, "extra");
    strcpy(label_keys, "extra");
    strcpy(label_params, "extra");

    // Parsing command line
    int res = parse_options(argc, argv, p, &flagverb_keys, &flagverb_ss, &flagverb_params, label_keys, label_ss, label_params);
    if (res == EXIT_FAILURE)
        return EXIT_FAILURE;
    ...
    if (flagverb_params == 1){
        print_gss_parameters(ss[0], label_params);
    }
