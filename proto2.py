...
def find_and_record_extrema_kernel(
    ...
    int_buf[idx, 2] = y
    int_buf[idx, 3] = x
    scale = delta_min * float(1 << o)
    float_buf[idx, 0] = float(y) * scale
    float_buf[idx, 1] = float(x) * scale
    exp_arg = float(o) + (float(s) / float(n_spo))
    float_buf[idx, 2] = float(sigma_min) * ld.exp2f(exp_arg)
    float_buf[idx, 3] = v
...
def refine_kernel(
    ...
    for _ in range(5):
        ...
        if off[1] > 0.6 and (y + 1) < (h - 1):
            y += 1
        if off[1] < -0.6 and (y - 1) > 0:
            y -= 1
        if off[2] > 0.6 and (x + 1) < (w - 1):
            x += 1
        if off[2] < -0.6 and (x - 1) > 0:
            x -= 1
        if off[0] > 0.6 and (s + 1) < (ns - 1):
            s += 1
        if off[0] < -0.6 and (s - 1) > 0:
            s -= 1
    ...
    if not (1 <= s < ns - 1 and 1 <= y < h - 1 and 1 <= x < w - 1):
        int_buf[idx, 0] = -1
        return
    D_hat = float(dog_oct[s, y, x]) + 0.5 * (
        g[0] * off[0] + g[1] * off[1] + g[2] * off[2]
    )
    int_buf[idx, 1], int_buf[idx, 2], int_buf[idx, 3] = s, y, x
    scale = delta_min * float(1 << o)
    float_buf[idx, 0] = (float(y) + off[1]) * scale
    float_buf[idx, 1] = (float(x) + off[2]) * scale
    exp_arg = float(o) + (float(s) + off[0]) / float(n_spo)
    float_buf[idx, 2] = float(sigma_min) * ld.exp2f(exp_arg)
...
def upscale(src, dst, delta_min, stream):
    ...
    oversample_bilinear_kernel[grid, (TX, TY), stream](src, dst, delta_min)
...
def discard_low_c(
    ...
    thresh = float(params.C_dog) * float(multiplier)
    threads = 128
...
@cuda.jit(device=True, inline=True, cache=True, fastmath=True)
def gaussian_weight(dx, dy, inv_2sig2):
    return ld.expf(-(dx * dx + dy * dy) * inv_2sig2)


@cuda.jit(device=True, inline=True, cache=True, fastmath=True)
def angle_bin_idx(angle, bin_scale, bins):
    bin_f = angle * bin_scale + 0.5
    idx = int(ld.floorf(bin_f)) % bins
    if idx < 0:
        idx += bins
    return idx
...
def window_bounds(y0, x0, R, h, w):
    ...
    return siMin, sjMin, siMax, sjMax


@cuda.jit(device=True, inline=True, cache=True, fastmath=True)
def smooth_orientation_hist(hist):
...
@cuda.jit(device=True, inline=True, cache=True, fastmath=True)
def trilinear_accumulate(hist, u, v, ob, wbase):
    u0 = int(ld.floorf(u))
    du = u - u0
    v0 = int(ld.floorf(v))
    dv = v - v0
...
def gradient_kernel(img, mag, ori):
    ...
    xp = w - 1 if x == w - 1 else x + 1
    ym = 0 if y == 0 else y - 1
    yp = h - 1 if y == h - 1 else y + 1

    fx = 0.5 if (0 < x < w - 1) else 1.0
    fy = 0.5 if (0 < y < h - 1) else 1.0

    gx = fx * (img[y, xp] - img[y, xm])
    gy = fy * (img[yp, x] - img[ym, x])
...
def orientation_kernel(
    ...
    if kp_idx >= n_extrema[0] or int_buf[kp_idx, 0] != oct_idx:
        return
    if cuda.threadIdx.x != 0 or cuda.threadIdx.y != 0:
        return
    ...
    bin_scale = ORI_BINS / TWO_PI

    hist = cuda.local.array(ORI_BINS, numba.float32)
    for i in range(ORI_BINS):
        hist[i] = 0.0
    ...
    for yy in range(siMin, siMax + 1):
        dyf = yy - y0
        for xx in range(sjMin, sjMax + 1):
            dxf = xx - x0
            m = mag[s, yy, xx]
            if m == 0.0:
                continue
            a = ori[s, yy, xx]
            wgt = m * gaussian_weight(dxf, dyf, inv_2sig2)
            idx = angle_bin_idx(a, bin_scale, ORI_BINS)
            hist[idx] += wgt

    smooth_orientation_hist(hist)
...
def descriptor_kernel(mag, ori, key_float, key_int, kctr, desc, oct_idx, delta_min):
    ...
    h, w = mag.shape[1:]
    siMin, sjMin, siMax, sjMax = window_bounds(y0, x0, Rp, h, w)

    c, snt = ld.cosf(theta0), ld.sinf(theta0)
    hist = cuda.local.array(DESC_LEN, numba.float32)
    for i in range(DESC_LEN):
        hist[i] = 0.0
    for yy in range(siMin, siMax):
        dy0 = yy - y0
        for xx in range(sjMin, sjMax):
            dx0 = xx - x0
...
def build_descriptors(
    ...
    orientation_kernel[(params.max_extrema,), (1, 1), stream](
    ...
    descriptor_kernel[(params.max_keypoints,), (1, 1), stream](
        data.mag[octave_index],
        data.ori[octave_index],
        data.keypoints.float_buffer,
        data.keypoints.int_buffer,
...
def compute_octave(
    ...
    snapshot["c_pre"] = discard_low_c(data, params, 0.8, octave_index, stream, record)
    snapshot["refined"] = refine_extrema(data, params, octave_index, stream, record)
    snapshot["c_post"] = discard_low_c(data, params, 1.0, octave_index, stream, record)
...
class Sift:
    ...
    def compute_many(self, img_paths: Iterable[str], record: bool = False):
        for p in img_paths:
            yield self.compute(p, record=record)
