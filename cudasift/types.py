...
def device_array(shape: tuple[int, ...] | int, dtype: Any) -> DeviceNDArray:
    """
    Typed wrapper for cuda.device_array to satisfy type checker.
    ...
    Returns:
        DeviceNDArray allocated on GPU
    """
...
class SiftParams:
    ...
    Attributes:
        ...
        record: Whether to record intermediate results during computation
        n_oct: Number of octaves (-1 for automatic)
        n_spo: Number of scales per octave
        sigma_in: Input image smoothing sigma
        delta_min: Minimum delta for scaling
        sigma_min: Minimum sigma for SIFT
        max_extrema: Maximum number of extrema to detect
        max_keypoints: Maximum number of keypoints to extract
        C_dog: Contrast threshold for DoG
        C_edge: Edge threshold
        lambda_ori: Lambda for orientation assignment
    """
    ...
    def __post_init__(self) -> None:
        """Initialize computed parameters after creation."""
    ...
    def _update_octave_count(self) -> None:
        """Automatically determine optimal number of octaves if not specified."""
    ...
    def _make_gss_shapes(self) -> np.ndarray:
        """Compute Gaussian scale space shapes for each octave."""
        base = np.array(
            [
    ...
    def _make_sigma_increments(self) -> np.ndarray:
        """Compute incremental sigmas for Gaussian blurring."""
    ...
    def _precompute_gaussian_kernels(self) -> None:
        """Precompute Gaussian kernels for all unique sigma values."""
...
class Extrema:
    ...
    Attributes:
        int_buffer: Integer data (octave, scale, y_int, x_int)
        float_buffer: Float data (y_world, x_world, sigma, dog_value)
        counter: Count and overflow tracking [count, overflow]
    """

    int_buffer: DeviceNDArray  # shape: (max_extrema, 4)
    float_buffer: DeviceNDArray  # shape: (max_extrema, 4)
...
class Keypoints:
    ...
    Attributes:
        int_buffer: Integer data (octave, scale, y_int, x_int)
        float_buffer: Float data (y_world, x_world, sigma, orientation)
        descriptors: 128-dimensional SIFT descriptors (uint8)
        counter: Count and overflow tracking [count, duplicates, overflow]
    """

    int_buffer: DeviceNDArray  # shape: (max_keypoints, 4)
    float_buffer: DeviceNDArray  # shape: (max_keypoints, 4)
    descriptors: DeviceNDArray  # shape: (max_keypoints, 128)
    counter: DeviceNDArray = field(
...
class KeypointsHost:
    ...
    int_buffer: np.ndarray  # shape: (max_keypoints, 4)
    float_buffer: np.ndarray  # shape: (max_keypoints, 4)
    descriptors: np.ndarray  # shape: (max_keypoints, 128)
    counter: np.ndarray = field(default_factory=lambda: np.zeros(3, dtype=np.int32))

    def copy(self) -> KeypointsHost:
        """Create a deep copy of the keypoints."""
...
class SiftData:
    ...
    Contains all GPU buffers needed for SIFT computation including:
    - Input/seed images
    - Gaussian scale space pyramids
    - Difference of Gaussians
    - Gradient images
    - Extrema and keypoint buffers
    """

    input_img: DeviceNDArray
    seed_img: DeviceNDArray
    scratch: tuple[DeviceNDArray, ...]
    gss: tuple[DeviceNDArray, ...]  # Gaussian Scale Space
    dog: tuple[DeviceNDArray, ...]  # Difference of Gaussians
    gx: tuple[DeviceNDArray, ...]  # X gradients
    gy: tuple[DeviceNDArray, ...]  # Y gradients
    extrema: Extrema
    keypoints: Keypoints
